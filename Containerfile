...
# ARG FEDORA_BASE_ARCH="${FEDORA_BASE_ARCH:-aarch64}"
FROM --platform=linux/arm64 ${BASE_IMAGE}:${FEDORA_MAJOR_VERSION} AS nokmods
...
# RUN wget https://copr.fedorainfracloud.org/coprs/ublue-os/staging/repo/fedora-$(rpm -E %fedora)/ublue-os-staging-fedora-$(rpm -E %fedora).repo?arch=aarch64 -O /etc/yum.repos.d/_copr_ublue-os_staging.repo && \
#     # wget https://copr.fedorainfracloud.org/coprs/kylegospo/oversteer/repo/fedora-$(rpm -E %fedora)-aarch64/kylegospo-oversteer-fedora-$(rpm -E %fedora).repo -O /etc/yum.repos.d/_copr_kylegospo_oversteer.repo && \
#     /tmp/install.sh && \
#     /tmp/post-install.sh && \
